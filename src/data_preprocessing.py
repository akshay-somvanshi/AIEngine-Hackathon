...
class CoffeeLLMDataPreparator:
    ...
    def load_coffee_dataset(self, file_path: str = "data/coffee_analysis.csv") -> pd.DataFrame:
    ...
    def extract_coffee_characteristics(self, description: str) -> Dict[str, List[str]]:
        ...
        mood_indicators = {
            "energizing": ["bold", "strong", "intense", "robust", "powerful", "vigorous", 
                          "lively", "bright", "sharp", "punchy", "invigorating"],
            "relaxing": ["smooth", "mellow", "gentle", "soft", "calm", "peaceful", 
                        "soothing", "mild", "delicate", "subtle", "silky"],
            "comforting": ["warm", "rich", "creamy", "cozy", "full-bodied", "enveloping",
                          "comforting", "embracing", "satisfying", "nurturing"],
            "sophisticated": ["complex", "nuanced", "elegant", "refined", "balanced",
                             "sophisticated", "layered", "intricate", "distinguished"],
            "adventurous": ["unique", "exotic", "unusual", "distinctive", "wild",
                           "adventurous", "experimental", "rare", "intriguing"]
        }
        
        flavor_indicators = {
            "chocolate": ["chocolate", "cocoa", "mocha", "dark chocolate", "milk chocolate"],
            "fruity": ["fruit", "berry", "cherry", "apple", "citrus", "orange", "lemon", "grape"],
    ...
    def process_unique_coffees(self, df: pd.DataFrame) -> Dict[Tuple[str, str], Dict[str, Any]]:
    ...
    def create_query_coffee_pairs(self, df: pd.DataFrame) -> List[Dict[str, Any]]:
        ...
        for coffee_key, coffee_data in unique_coffees.items():
            coffee_embedding = self.model.encode([coffee_data["description"]])[0].tolist()
        ...
        for query_type, queries in self.query_templates.items():
            ...
            for query in queries:
                ...
                for coffee_key, coffee_data in unique_coffees.items():
                    ...
                    if not is_match and random.random() < 0.1:  # 10% chance for diversity
                        is_match = True
    ...
    def find_best_matches(self, query: str, top_k: int = 3) -> List[Tuple[float, Dict[str, Any]]]:
    ...
    def create_llm_training_format(self, training_pairs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Convert training pairs to LLM fine-tuning format with embeddings as features."""
        llm_training_data = []
        
        for pair in training_pairs:
            # Create input with embedding features
            input_data = {
                "query": pair["query"],
                "query_embedding": pair["query_embedding"],
                "task": "recommend_coffee_from_mood_and_taste_preferences"
            }
            ...
            output_data = {
                "recommendation": {
                    ...
                    "coffee_embedding": pair["coffee_embedding"]
                }
            }
            
            # Format for LLM training (instruction-following format)
            llm_example = {
                "instruction": "Given a user's coffee preference query and its embedding representation, recommend a suitable coffee with explanation.",
                "input": json.dumps(input_data),
                "output": json.dumps(output_data),
                "metadata": {
                    "query_type": pair["query_type"],
                    "coffee_characteristics": pair["coffee_characteristics"]
                }
            }
    ...
    def create_embedding_features_dataset(self, training_pairs: List[Dict[str, Any]]) -> pd.DataFrame:
        ...
        for pair in training_pairs:
            row = {
                ...
                "coffee_description": pair["coffee_description"][:200],  # Truncate for readability
            }
            ...
            similarity = np.dot(query_emb, coffee_emb) / (np.linalg.norm(query_emb) * np.linalg.norm(coffee_emb))
    ...
    def save_training_data(self, training_pairs: List[Dict[str, Any]], output_dir: str = "data"):
        ...
        unique_coffees_list = [coffee_data for coffee_data in self.unique_coffees.values()]
        ...
        print(f"- Embedding features CSV: {embedding_df.shape[0]} rows x {embedding_df.shape[1]} columns")
    ...
    def analyze_embeddings(self, training_pairs: List[Dict[str, Any]]):
        ...
        for qtype in unique_types:
            type_similarities = [s for s, t in zip(similarities, query_types) if t == qtype]
            if type_similarities:  # Check if list is not empty
                print(f"- {qtype}: {np.mean(type_similarities):.3f} ± {np.std(type_similarities):.3f}")
    ...
    def test_embedding_quality(self, training_pairs: List[Dict[str, Any]]):
            ...
            for j, q2 in enumerate(test_queries):
                if i != j:
                    similarity = np.dot(query_embeddings[i], query_embeddings[j]) / \
                               (np.linalg.norm(query_embeddings[i]) * np.linalg.norm(query_embeddings[j]))
        ...
        for query in sample_queries:
            ...
            for i, (sim, coffee_data) in enumerate(matches):
                print(f"  {i+1}. {coffee_data['name']} by {coffee_data['roaster']} ({sim:.3f})")
    ...
    def quick_embedding_check(self, training_pairs: List[Dict[str, Any]]):
        ...
        print(f"Coffee: {sample_pair['coffee_name']} by {sample_pair['coffee_roaster']}")
